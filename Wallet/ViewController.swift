...
class ViewController: UIViewController {
    ...
    @IBAction func Authenticate(_ sender: Any) {
        ...
        if context.canEvaluatePolicy(.deviceOwnerAuthenticationWithBiometrics, error: &error) {
            context.evaluatePolicy(LAPolicy.deviceOwnerAuthenticationWithBiometrics, localizedReason: "Please Identify Yourself to Access Wallet") {
                ...
                DispatchQueue.main.async {
                    if success {
                        ...
                        let alert = UIAlertController(title: "Authentication Failed", message: "Failed to authenticate user", preferredStyle: .alert)
            ...
            let alert = UIAlertController(title: "No Biometric Authentication", message: "This app only works on devices with biometric authentication capability", preferredStyle: .alert)
            alert.addAction(UIAlertAction(title:"Got It!", style: .default, handler: nil))
            self.present(alert, animated: true, completion: nil)
        }
    }
